...
import { DocumentAttachmentApiService, DocumentTemplatesApiService, QuoteApiService } from '@veloceapps/api';
import { Attachment, DocumentTemplate, QuoteDraft, TemplateProperty, TemplatePropertyLov } from '@veloceapps/core';
import { ElementDefinition, IntegrationState, ScriptHost } from '@veloceapps/sdk/cms';
...
interface ScriptContent {
  ...
  attributes$: Observable<TemplateAttribute[]>;
  attributesForm: FormGroup;
  attachments$: BehaviorSubject<Attachment[]>;
...
interface TemplateAttribute extends Omit<TemplateProperty, 'lov'> {
  lov: TemplatePropertyLov[];
}
...
export class Script implements OnInit, OnDestroy {
  ...
  ngOnInit(): void {
    ...
    this.host.attributes$ = this.selectedTemplate$.pipe(
      ...
      map(attributes =>
        attributes.map(prop => {
          return { ...prop, lov: this.parseLov(prop.lov ?? []) };
        }),
      ),
    );
  ...
  private initAttributesForm = (attributes: TemplateAttribute[]): void => {
    ...
    this.host.attributesForm.valueChanges
      .pipe(takeUntil(this.destroy$), startWith(this.host.attributesForm.value))
      .subscribe(value => {
        this.integrationService.dispatch({ type: IntegrationAction.DOCGEN_ATTRIBUTES_UPDATED, payload: value });
      });
  };

  private initAttachmentsForm = (attachments: Attachment[]): void => {
  ...
  private addAttachmentControl = (attachment: Attachment): void => {
    const controls = {
  ...
  private parseLov = (lov: string[]): TemplatePropertyLov[] => {
    return lov.map(value => this.parseJsonSafe(value) ?? { code: value, value });
  };
  ...
  private fileSelectHandler = (event: Event): void => {
    ...
    this.templatesService
      .createAttachment({ linkedEntityId: this.host.recordId } as Attachment, file)
      .pipe(
  ...
  private parseJsonSafe(jsonStr: string): any | undefined {
    try {
